...
<span style="color: brown">Esta práctica actuarial es un taller enel que desarrollaras  modelos actuariales para la toma de decisiones, considerando como herramienta el lenguaje R, en esta ocasión en torno a temas de pricing y seguimiento de un portafolio de vida individual, tomando la problemática que representan las tasas de interés bajas.
...
+ <span style="color: brown">**Lecturas y Ejercicios en R:** Constituidas esta aplicación en la que encontraras una serie de lecturas (en café) y bloques de ejercicios en R (en azul)  
+ <span style="color: brown">**Espacio colaborativo de la práctica** Medio que te permitirá plantear tus dudas y sugerencias y lo mas importante, contribuir en un proyecto colectivo que fortalezca la practica actuarial en español.  


<span style="color: brown">Para participar en este  espacio  necesitas dar de alta una cuenta de GitHub y buscar "DeltaWiseC" o "Practicas actuariales"</span>

<span style="color: brown">El objetivo de las lecturas es darle contexto a los elementos técnicos y revisar la situció de la problemática del tema de la práctica. 
...
## <span style="color: navy">E-1: Variables y Operadores{data-progressive=TRUE}


<span style="color: navy">Una vez que sabemos cómo manejar las secciones de código, *!empecemos con los ejercicios!*

<span style="color: navy">En esta sección vas a crear e inicializar variables en R,  utilizarás operadores aritméticos y lógicos y conoceras algunas funciones generales.


```{r EJER01, exercise=TRUE,exercise.cap="Variables y operadores aritmeticos"}

# Una variable en R es un espacio de memoria que se puede recuperar mediante un nombre 
# y que conserva diferentes atributos, entre ellos un valor

# Para crear una variable y asignarle un valor R utiliza la sintaxis "<-", tambien es factible utilizas "->"

# Lo interpretaremos como a la variable 'NOMBRE' se le asigna 'VALOR'

tasa <- 0.04  # Asigna el valor de 0.04 a una espacio de memoria que recuperar con la referencia "tasa" 
...
# De la formula de la anualidad obten la expresión para determinar la renta necesaria para llegar a un VP dado
...
# Despliega en la pantalla el resultado de renta y VPf

<< Tu código >>

# Recuerda la sintaxis de las operaciones aritméticas  
A <- 4  ; B <- 2
#     Suma          Resta         Multiplicación 
  Res1 <- A+B ; Res2 <- A - B   ; Res3 <- A*B 

#  División       Parte Entera       Modulo
 Res4 <- A/B ; Res5 <- A %/% B ; Res6 <- A %% B
...
<span style="color: navy">Cuando la tasa de interés es una incógnita se puede utilizar una técnica de interpolación para determinar la tasa que cumple con la ecuación, en este caso vamos a realizar este proceso de forma gráfica (identifica el comando). 

```{r tasa00, exercise=TRUE, exercise.cap="Operadore aritmeticos, graficos básicos"}
# En este caso la variable tasa tiene varios valores, mas adelante lo definiremos como un vector
...
# las formulas aritmeticas aceptan vectores y en este caso genera un vector de Valores presentes
VP <- pago*(1-(1+tasa)^(-plazo))/tasa 

# Grafica los valores presentes
plot(tasa,VP, xlab="Tasa de Interes",ylab="Valor Presente",main="Grafica")
abline(a=10000,b=0,col="red")


# Una forma de inicializar variables es asignandole el resultado de una 
# función, en este caso vamos a utilizar la función seq "sequence" para 
# inicializar a variable, sustituye la asignación inicial de la variable tasa por
# la siguiente expresión y corre nuevamente el código:
# tasa<-seq(from=0.0495,to=0.051, by=0.0001)

# También puedes modificar los parámetros de la gráfica, títulos, colores

# Corre nuevamente el codigo
...
```{r Ejer10, exercise=TRUE,exercise.cap="Operadores Lógicos"}

# R puede hacer comparaciones logicas entre las variables el resultado es TRUE-VERDADERO / FALSE-FALSO

Compara <- pi < exp(1)

paste("valor de Compara ", Compara, "  negación de Compara ", !Compara)

# Recuerda la sintaxis de las comparaciones lógicas 
A<-4  ; B<-2

#     Mayor     Mayor o Igual        Menor  
  Log1 <- A > B; Log2 <- A >= B; Log3 <- A < B
  
#  Menor o igual       Igual         Diferente   
   Log4 <- A<=B ; Log5 <- A == B; Log6 <- A != B 
...
_<span style="color: navy">La práctica hace al maestro, reza el refrán, sigue en esa línea, **descubre el potencial de R**.
...
# Operatividad de la funcion c(. . .), combine r
# Graficas basicas de linea (x,y) "plot"" y lineas horizontales "abline""
# Uso de la función sequence  "seq"
...
<img src="images/EvolMort.png" width="800"/>


<img src="images/ComparativoMortalidad.png" width="800"/>
...
## E-2: <span style="color: navy">Vectores {data-progressive=TRUE}

<span style="color: navy">En esta sección realizaremos ejercicios con vectores, entenderemos por vector la representación de una variable que tiene multiples valores de una misma clase
...
```{r duda4010, exercise.cap="Operación de Vectores "}
question("Si A<- c(2,6,1,7,8,4) B<-c(5,3,4) C<- 7 cuales de las siguientes expresiones corresponden con el concepto: ",
         answer("i)  (A => B   Entrega un vector logico comparando las componentes de A con las de B",message="i)  No corresponde, de hecho entrega un error el operador => no existe <br>"),
         answer("ii)  (A %/% B   Entrega un vector numérico con la parte entera de la división de A entre B, repite los valores de B ",correct=TRUE,message="ii)  Correcto, el operador %/% representa la división entera, como B tienen menos componentes que A, los componentes de B se 'reutilizan´para completar la operación <br>"),
         answer("iii)  (B %*% B   Entrega un valor numérico con la suma de los cuadrados del vector B",correct=TRUE,message="iii)  Correcto, el operador %*% producto punto multiplica los vectores componente a componente y suma estos productos <br>"),
         answer("iv)  (A <- C  Compara cada componente de A con el valor de C",message="iv)  In correcto, de hecho esta expresión esta asignando el valor C a la variable A <br>"),
         answer("v)  (A %in% C  Es un error porque el vector A tiene mas componentes que el vector C",message="v)  Incorrecto, esta expresión es valida y entrega un vector logico en el que establece si cada componente de A esta incluida en C <br>"),
...
question("Dado que A<-c(4,8,12,20) B<-c(3,,9,11,2) C<-c(1,0,-4,7) cuales de las siguientes expresiones son válidas, sin modificar el contenido de los vectores, marca todas las expresiones válidas ",
         answer("i)  D<-C[A<=B]",correct=TRUE,message="i) Regresa el componente de C en los que el componente de A es menor que el de B.  <br> "),
         answer("ii)  D<- A U B ", message="ii)  No hay ninguna instrucción o función en el comando el simbolo U es tomado como una variable.<br>"),
         answer("iii)  D<-c(A,B,C)", correct=TRUE,  message="iii)  Combina los tres conjuntos.<br>"),
         answer("iv)  D<-C(A,B)",message="iv)  Los paréntesis redondos de utilizan para llamar una función, la función C no existe. <br>"),
         answer("v)  D<-C[A,B]",message="v)  El vector C solo tiene una dimensión, el llamado [A,B] hace referencia a una matriz. <br>"),
...
question("Dados los vectores A<-c(a1,a2,a3,a4) D<-c(d1,d2) R<-c(r1,r2) cuales son expresiones validas para que el vector D opere como un factor de descuento a las primeras dos componentes de A y el vector R opere como un recargo a las ultimas componentes de A, al vector S se le asigna la solución, marca todas las expresiones válidas ",
         answer("i) S<-A*c(1-D,1+R)",correct=TRUE,message="i) Integra un vector de descuentos y recargos de cuatro componentes y lo opera con A .....<br>"),
         answer("ii) S<-A[1:2]*(1-D)+A[3:4]*(1+R)", message="ii) Este comando genera un vector de 2 componenetes que suma las dos primera componentes de A descontadas por D y los ultimas dos recargadas por R.....<br>"),
         answer("iii) S<-c(A[1:2]*(1-D),A[3:4]*(1+R))",correct=TRUE, message="iii) Determina las componentes de A con descuento y recargo y las combina en un vector de 4 componenetes.....<br>"),
         answer("iv) S <- c(A[1]*(1-C[1]),A[2]*(1-C[2]),A[3]*(1+R[1]),A[4]*(1+R[2]))", correct=TRUE,  message="iv) Cada componente de A se afecta por el descuento/recargo correspondiente.....<br>"),
         answer("v) S<-c(A[1:2]*D),A[3:4]*R))",message="v) No determina los factores de descuento / recargo.....<br>"),
         allow_retry = TRUE,
         random_answer_order = FALSE
         )
...
<span style="color: navy">El objeto TablaMx  esta compuesto
...
<span style="color: navy">Producto punto:&nbsp; &nbsp; $X$\%*\%  $Y$=$x_1*y_1+\dots+x_n*y_n$  
<span style="color: navy">Producto estándar:  $X$\*$Y$=($x_1*y_1,\dots,x_n*y_n)$
...
### 2.5.2 <span style="color: navy">Reserva, Valores en efectivo y Rescates

<span style="color: navy">En este ejercicio calcularemos las reservas de una póliza para un año especifico, no determinaremos el efecto de las reservas sobre las primas, ya que para ello necesitaríamos establecer el requerimiento de capital y un objetivo de rentabilidad.

<span style="color: navy">A pesar de que no revisaremos la interacción de las reservas con las primas si es relevante tener presente las siguientes consideraciones conceptuales.

<span style="color: navy">Si bien las reservas pueden considerarse independientes del ejercicio de pricing, ya que incluso se calculan con una tabla de mortalidad recargada y una tasa de interés más baja,  tienen un efecto considerable en el requerimiento de capital, la utilidad esperada y por lo tanto en la rentabilidad del plan, como consecuencia deben incluirse en la construcción de un producto.

<span style="color: navy">El plazo de seguro y el objetivo de compra de la póliza determinan el horizonte de inversión e influyen en el comportamiento del asegurado, en cuanto a ejercer las diferentes opciones implícitas en la póliza, lo anterior conforma la estructura de las reservas y es un factor relevante para el calculo del capital requerido en la operación de vida.

<span style="color: navy">Así mismo el patrón de desarrollo de las reservas puede representar un problema de liquidez cuando se presenta una discrepancia temporal entre flujos de entrada y de salida en los casos que los flujos de salida son mayores, ya que la empresa debe compensar la diferencia con capital. 

<span style="color: navy">Por último hay que considerar que la cancelación anticipada de las pólizas representa una fuente de ingresos, debido a los cargos por rescate. 

<span style="color: navy">Nos valdremos de la siguiente expresión para determinar la reserva al año t:

<span style="color: navy">$_{t}V_x = A_{x+t} - P_x\ddot{a}_{x+t}$

<span style="color: navy">Y utilizaremos nuevamente la solución vectorizada
...
<span style="color: navy">Con esto llegamos a la conclusión del segundo apartado y es tiempo de hacer un recuento de lo conceptos que se presentan en este capitulo
...
<details> <summary><span style="color: brown">Causas  </summary>
...
<details> <summary><span style="color: brown">En México</summary>

<img src="images/NalTendB.png" width="800"/>

<img src="images/NalTendU.png" width="800"/>

</details>

<details> <summary><span style="color: brown">En el mundo</summary>

<img src="images/InterTendencia.png" width="800"/>
...
<details> <summary><span style="color: brown">Diseño de productos</summary>
...
<details> <summary><span style="color: brown">Productos Unit Linked</summary>
...
<details> <summary><span style="color: brown">Hedging</summary>
...
<details> <summary><span style="color: brown">Calce entre Activos y Pasivos</summary>
...
<span style="color: navy">Para variable unitarias se puede omitir la sección de falso y por la tanto el comando "else"
...
```
### <span style="color: navy"> 3.2 función **for** 
...
question("Seleccione todos los errores de sintaxis:",
         answer("i)   La condición de if debe indicarse entre paréntesis",correct=TRUE, message="i)   Bien, la sintaxis es if (cond) {} else {}  <br>"),
         answer("ii)  El operador lógico de igualdad es incorrecto",correct=TRUE, message="ii)  EL operador correcto es ==  <br>"),
          answer("iii) La expresión !condicion es incorrecta",message="iii) EL operador ! es negación<br>"),
         answer("iv) El operador logico de menor o igual es incorrecto<br>",correct=TRUE, message="iv) EL operador correcto es <=  "),
         answer("v)   La expresión false despues de else debe indicarse entre corchetes", message="v)  No necesariamente, si es una expresion unica puede ir sin corchetes<br> "),
         answer("vi) No es factible cambiar el valor de epsilon porque forma parte de la condicion", message="vi) La evaluación de la condición se realiza con el valor de las variables en ese punto del algoritmo, posteriormente puede modificarse cualquier variable<br>"),
         allow_retry = TRUE,
         random_answer_order = FALSE
         )
```
...
question("Cuales son las estrategias operativas que se reflejan en el modelo de ruina ",
         answer("i)   Realizar incrementos de capital ",correct=TRUE),
         answer("ii)  Modificar la política de pricing, considerar un margen de seguridad por arriba de los siniestros esperados", correct=TRUE),
         answer("iii) Modificar la política de reaseguro", message="iii) Si bien influye en el resultado del ejercicio no esta considerada en el modelo "),
         answer("iv)  Modificar la política comercial, enfoque a nichos de SA mas baja", message="iv)  Resultado incierto, puede afectar los siniestros pero tambien la prima, solo tiene efectos positivos si reduce la desviación estándar (volatilidad) de los siniestros" ),
         answer("v)   Modificar la política de suscripción, evitar riesgos subnormales ", correct=TRUE, message="v)   Si tiene efecto sobre la desviación estándar de los siniestros"),
...
<span style="color: navy">Variables de entrada
  ...
  - <span style="color: navy">Edad del solicitante, determina el patron de reclamaciones y pago de primas   - <span style="color: navy">Sexo del solicitante, en caso de que se realicen descuentos en la edad de calculo  
...
<details> <summary><span style="color: brown">Servicios que requiere una póliza</summary>
...
**<span style="color:brown">Gastos de Adquisición:** Incluyen costos incurridos por suscribir un nuevo contrato, vamos a clasificarlos: 
...
## <span style="color: navy">E-4: Funciones {data-progressive=TRUE}

<span style="color: navy">En esta sección desarrollaremos funciones definidas por nosotros, esto con el objeto de aprovechar el codigo que hemos desarrollado y aplicarlo con otros parametros, con ello vamos a ahorrar mucho tiempo en la codificación y evitaremos posibles errores al reescribir secciones de codigo.

<span style="color: navy">Veamos nuestro primer ejemplo:
...
# - Si en la definición de la funcion, se declara un valor por omisión para un parámetro (en este ejemplo t=4) 
# es factible omitir este parámetro de la llamada
# - Una función puede entregar diferentes resultados dependiendo de los parámetros 
...
<span style="color: navy">El siguiente ejemplo determina las tasas de los instrumentos libres de riesgo para un periodo continuo de tiempo.

<span style="color: navy">Recordemos que **la tasa libre de riesgo**, es la alternativa que tiene el inversionista para colocar sus recursos sin asumir ningún riesgo crediticio ni riesgo de reinversión, esto es que la entidad que emitió el documento siempre va a cumplir con sus compromisos y que al vencimiento del documento va a estar disponible otro documento con al menos los mismos rendimientos.

<span style="color: navy">Para el caso del mercado mexicano la tasa libre de riesgo esta determinada por la tasa que pagan los documentos gubernamentales, para los instrumentos en moneda nacional, tenemos:
...
<span style="color: navy"><font size ="1"> Fuente:www.worldgovermentbonds.com</font>  

<span style="color: navy">Las tasas libres de riesgo, se utilizan en los modelos de seguros para descontar los flujos de la empresa y determinar la rentabilidad del negocio, entre otras cosas.

<span style="color: navy">El problema que se presenta es que la curva de tasas tiene ciertos nodos, en este caso 1,3 5, 10, 20 y 30 años, mientras que el portafolio tiene vencimientos todos los años.

<span style="color: navy">Para ello tomaremos los nodos disponibles y proyectaremos las tasas de los plazos intermedios, esto lo haremos con una interpolación lineal.
...
<span style="color: navy">En esta sección vamos a retomar el código para el cálculo de la prima de riesgo y de tarifa y lo vamos a declarar como una función con el objeto de calcular estas primas para un conjunto de edades, asi mismo, habilitaremos el campo de tasa de Interés para que pueda recibir como parámetro un vector y calcular los flujos con una curva de tasas de Interés.
...
question("Cuales son las estrategias operativas que se reflejan en el modelo de ruina ",
         answer("i)    Realizar incrementos de capital ",correct=TRUE),
         answer("ii)   Modificar la política de pricing, considerar un margen de seguridad por arriba de los siniestros esperados", correct=TRUE),
         answer("iii)  Modificar la política de reaseguro", message="iii) Si bien influye en el resultado del ejercicio no esta considerada en el modelo <br>"),
         answer("iv)   Modificar la política comercial, enfoque a nichos de SA mas baja", message="iv)   Resultado incierto, puede afectar los siniestros pero tambien la prima, solo tiene efectos positivos si reduce la desviación estándar (volatilidad) de los siniestros <br>" ),
         answer("v)    Modificar la política de suscripción, evitar riesgos subnormales ", correct=TRUE, message="v)    Si tiene efecto sobre la desviación estándar de los siniestros <br>"),
...
## <span style="color: brown">L-5: Caso: Seguros El Fenix {.tabset}

### <span style="color: brown">La Cartera

<span style="color: brown">A lo largo de los años y administraciones, la compañia de Seguros FENIX ha consolidado un portafolio de alrededor de 100 mil polizas de vida individual, caracterizado por cuatro bloques de negocio
...
#### <span style="color: chocolate">Gen X</span>  
<details> <summary>Click para expandir/contraer!</summary>

* <span style="color: brown">**Mercado objetivo:** Población economicamente activa, con capacidad de ahorro de al menos 10% de su ingreso, oferta comercial en planes con un fondo asociado, participación de utilidades sobre reservas. Distribución por sexo 70% masculino, 30% femenino, suma asegruada promedio $300,000 Mx

* <span style="color: brown">**Estructura de planes:**
    - Temporal 20                60%  
...
## <span style="color: navy">E-5: El reto de las tasas de interés bajas {data-progressive=TRUE}

<span style="color: navy">Hasta ahora hemos construido un modelo de pricing y un modelo de cash flow, ambos aplican en el diseño de un plan, en ese momento hicimos nuestra mejor estimación sobre los diferentes parámetros que determinan el plan de tal forma que cumplen con  los objetivos de los diferentes participantes, en resumen podemos decir que construir un **modelo actuarial** es hacer una representación de la realidad identificando factores e interacciones entre ellos que nos llevan a un resultado determinado,   establecer sus relaciones, construir métricas de seguimiento, identificar y analizar tendencias son tareas que combinan  ciencia, tecnología y arte, encontrar matices, momentos, ritmos y perspectivas, mezclarlo con algoritmos, herramientas, teoremas y probabilidades es un reto que vale la pena tomar.

<span style="color: navy">Como mencionamos al momento del diseño del plan hicimos nuestra mejor estimación de los parámetros técnicos, fuimos prudentes determinando el valor de cada uno de ellos para manejar los riesgos dentro del apetito de riesgo establecido y fuimos lo suficientemente agresivos para posicionar el producto como atractivo para los asegurados y canal de distribución. Pese a todos nuestros esfuerzos y buena disposición lo anterior no es suficiente ya que el entorno de negocio es dinámico y debemos tener una idea del comportamiento de nuestra cartera con la situación actual del entorno de negocio.

<span style="color: navy"> De esta forma tenemos dos situaciones fundamentales que debemos atender

i) <span style="color: navy">Administrar un portafolio de pólizas  
ii)<span style="color: navy"> Desarrollar un nuevo plan

<span style="color: navy">En este ejercicio vamos utilizar el ejercicio de Cash FLow y aplicarlo al portafolio comparando los resultados utilizando los parámetros de origen,los actualizados y los parámetros esperados, haciendo especial énfasis en las tasas de interés.
...
C.D. Daykin 1984 "Practical Risk Theory for Acturies" Chapman & Hall

<a href = "https://forms.office.com/Pages/ResponsePage.aspx?id=UJtdQLAA1USJ3jk9U1VhOWgA-JUoeRNHr_OuvblySZ5UQUJONVFWTUMzUTg0SE5SM0hNTENCNVJYUy4u" target="_blank">Danos la oportunidad de mejorar, agradecemos tu retroalimentación</a>
